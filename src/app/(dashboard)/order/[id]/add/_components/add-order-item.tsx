...
import { addOrderItems } from "../../../action";
...
export default function AddOrderItem({ id }: { id: string }) {
  ...
  const { data: menus, isLoading: isLoadingMenu } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.error)
        toast.error("Get Manu data Failed", {
  ...
  const handleAddToCart = (menu: Menu, action: "increment" | "decrement") => {
    const existingItems = carts.find((item) => item.menu_id === menu.id);
  ...
  const [addOrderItemState, addOrderItemAction, isPandingAddOrderItem] =
    useActionState(addOrderItems, INITIAL_STATE_ACTION);
    ...
    <div className="flex flex-col lg:flex-row gap-4 w-full">
      <div className="space-y-4 lg:w-2/3">
        ...
        {!isLoadingMenu && menus?.data?.length === 0 && (
          <div className="text-center w-full">No menu found</div>
        )}
      </div>
      <div className="lg:w-1/3">
        <CardSection
          ...
          setCarts={setCarts}
          onAddToCart={handleAddToCart}
          isLoading={isPandingAddOrderItem}
          onOrder={handleOrder}
        />
      </div>
    </div>
